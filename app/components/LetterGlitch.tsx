...
const LetterGlitch = ({
  ...
  const resizeCanvas = () => {
    ...
    if (context.current) {
      (context.current as CanvasRenderingContext2D).setTransform(dpr, 0, 0, dpr, 0, 0);
    }
  ...
  const updateLetters = () => {
    ...
    for (let i = 0; i < updateCount; i++) {
      ...
      const letter = letters.current[index] as Letter;
  ...
  useEffect(() => {
    ...
    context.current = (canvas as HTMLCanvasElement).getContext('2d');
    resizeCanvas();
    animate();
